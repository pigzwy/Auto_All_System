...
Browser Pool 在获取 profile 时会自动处理指纹：

- **首次创建**：调用 `generate_fingerprint()` 生成随机指纹（包含 Canvas/Audio 噪声、屏幕分辨率、CPU/内存等）
- **后续复用**：保持原有指纹不变，只更新 proxy

这样设计的好处：
- 避免风控系统检测到"同一浏览器频繁变化参数"
- 减少创建 profile 时的网络开销
- 提高自动化任务稳定性

### 启动参数

Profile 创建时会自动注入反检测参数：
...
该参数用于隐藏 `navigator.webdriver` 等自动化特征。

> 详细指纹参数说明见：[11-GeekezBrowser_API_变更适配说明.md](./11-GeekezBrowser_API_变更适配说明.md#风控指纹参数说明)
