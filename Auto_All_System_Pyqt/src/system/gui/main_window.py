...
class MainWindow(QMainWindow):
    ...
    def _set_icon(self):
        """设置窗口图标"""
        try:
            # 优先使用 ico 文件作为窗口图标（兼容性更好）
            ico_path = resource_path("app_icon.ico")
            if os.path.exists(ico_path):
                self.setWindowIcon(QIcon(ico_path))
            else:
                # 备用：使用 svg 文件
                svg_path = resource_path("beta-1.svg")
    ...
    def _action_verify_sheerid(self):
        ...
        if not api_key:
            QMessageBox.warning(
                self, "提示", 
                "未配置 SheerID API Key\n\n"
                "请先在 Web管理界面 或 系统设置 中配置 API Key\n"
                "(从 batch.1key.me 获取)"
            )
            return
        ...
        msg += f"API Key: {api_key[:10]}...{api_key[-4:]}"
        reply = QMessageBox.question(
            self, '确认操作', msg,
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        ...
        self.log(f"🔑 使用API Key: {api_key[:10]}...{api_key[-4:]}")
        
        self._stop_flag = False
        self.btn_stop.setEnabled(True)
        
        from gui.worker_thread import WorkerThread
        self._worker = WorkerThread('verify_sheerid', ids=verification_ids, api_key=api_key)
        self._worker.log_signal.connect(self.log)
