"""
@file sheerid_gui.py
@brief SheerID批量验证工具GUI
@details 支持批量验证SheerID链接，显示验证进度和结果
"""
...
# 使用新的模块路径导入
try:
    from ..backend import SheerIDVerifier, AccountManager
...
class VerifyWorker(QThread):
    ...
    def __init__(self, api_key: str, links: list, thread_count: int = 1):
        """
        @brief 初始化验证工作线程
        @param api_key SheerID API密钥
        @param links 待验证链接列表 [{'vid': '...', 'line': '...'}, ...]
...
class SheerIDWindow(QDialog):
    ...
    def _extract_vid(self, line: str) -> str:
    ...
    def _start_verify(self):
        ...
        self.worker.progress_signal.connect(self._update_row_status)
    ...
    def _update_row_status(self, data: dict):
        """更新行状态"""
        vid = data['vid']
        status = data['status']
        msg = data['msg']
    ...
    def _cancel_selected(self):
        ...
        reply = QMessageBox.question(
            self, "确认", f"确定取消 {len(checked_rows)} 个任务吗？", 
