...
class GoogleLoginService:
    ...
    async def login_with_playwright(
        ...
        if status == GoogleLoginStatus.LOGGED_IN and not force_login:
            ...
            if logged_email:
                if logged_email.lower() == email.lower():
                    self.log(f"已登录正确账号: {logged_email}")
                    return True, "already_logged_in", f"已登录: {logged_email}"
                else:
                    self.log(f"当前登录账号 {logged_email} 与目标 {email} 不符")
                    # 需要切换账号，先登出
        ...
        if success:
            ...
            final_status, final_info = await check_google_login_status(page)
            
            if final_status == GoogleLoginStatus.LOGGED_IN:
                logged_email = final_info.get('email', email)
